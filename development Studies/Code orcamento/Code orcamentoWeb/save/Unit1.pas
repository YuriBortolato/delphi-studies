...
begin
  ...
  try
    // Passa o preço que está na memória para a janela
    FrmDesconto.ValorOriginal := PrecosAtuais[TagID];

    // Abre a janela e espera o usuário confirmar
    if FrmDesconto.ShowModal = mrOk then
    begin
      // 1. Atualiza o preço na memória
      PrecosAtuais[TagID] := FrmDesconto.ValorFinal;

      // 2. Atualiza o visual do Label
      LblClicado.Caption := FormatFloat('R$ #,##0.00', PrecosAtuais[TagID]);

      // 3. Lógica visual: Se o preço novo for menor que o original daquele módulo
      // Precisamos comparar com o preço base do módulo.
      // Como simplificamos, vamos comparar com o valor que estava antes de abrir a janela.

      if FrmDesconto.ValorFinal < FrmDesconto.ValorOriginal then
      begin
        LblClicado.Font.Color := clGreen; // Fica VERDE
        LblClicado.Font.Style := [fsBold]; // Fica NEGRITO
      end
      else
      begin
        LblClicado.Font.Color := clWindowText; // Volta ao preto normal
        LblClicado.Font.Style := []; // Tira negrito
      end;

      // 4. Recalcula o total geral
      CalcularTudo;
    end;
  finally
    FrmDesconto.Free;
